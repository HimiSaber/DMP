    ...
    /**
      ...
      */
        ...
        val rdd: RDD[(String, List[Double])] = df.map(arr => {
          ...
          //key
          val ispname: String = arr.getAs[String]("ispname")

          (ispname, RptUtils.request(reqm, proc) ++ RptUtils.click(reqm, iseff) ++ RptUtils.Ad(iseff, isbil, isbid, iswin, ado, winpri, adpay))
        }).rdd
