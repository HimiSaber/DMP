...
import com.ProCityCt.proCity.getProperties
...
object LocationRpt {
  def main(args: Array[String]): Unit = {
    ...
    val ret: DataFrame = ssc.sql("select provincename,cityname," +
      ...
      " from (select provincename,cityname," +
      ...
      "from t_log)ret group by provincename,cityname   ")
    ret.show()
    ret.write.mode(SaveMode.Append)
      .jdbc(getProperties()._2,"LocationRpt",getProperties()
        ._1)
...
//
//      //key
//      val pn: String = arr.getAs[String]("provincename")
//      val cn: String = arr.getAs[String]("cityname")
//      ((pn, cn), RptUtils.request(reqm, proc) ++ RptUtils.click(reqm, iseff) ++ RptUtils.Ad(iseff, isbil, isbid, iswin, ado, winpri, adpay))
//    }).rdd
//
//    val ret: RDD[((String, String), List[Double])] = rdd.reduceByKey((a: List[Double],b: List[Double])=>(a zip b).map(tup=>tup._1+tup._2))
//
//    //rdd.map(_._2.size).take(20).foreach(println)
//
//    val res: RDD[(String, String, Double, Double, Double, Double, Double, Double, Double, Double)] = ret.map(a=>(a._1._1,a._1._2,a._2(0),a._2(1),a._2(2),a._2(5),a._2(6),a._2(3),a._2(4),a._2(7)))
//
