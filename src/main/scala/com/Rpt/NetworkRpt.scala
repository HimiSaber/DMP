  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      */
        ...
        val ret: DataFrame = ssc.sql("select case when networkmannername='未知' then '其他' else networkmannername end `网络`," +
          ...
          "from t_log)ret group by networkmannername   ")
        ret.show()
        ret.write.mode(SaveMode.Append)
          .jdbc(getProperties()._2,"NetWork",getProperties()
            ._1)
...
//
//      //key
//      val networkmannername: String = arr.getAs[String]("networkmannername")
//
//      (networkmannername, RptUtils.request(reqm, proc) ++ RptUtils.click(reqm, iseff) ++ RptUtils.Ad(iseff, isbil, isbid, iswin, ado, winpri, adpay))
//    }).rdd
//
...
//  def unkown2other(a:String):String={
//    val ret:String = if(a.equals("未知")) "其他" else a
//    ret
//  }
}
