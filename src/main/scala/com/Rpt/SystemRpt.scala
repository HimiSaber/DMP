...
//
//    val ret: DataFrame = ssc.sql("select case client when 1 then 'Android' when 2 then 'IOS' when 3 then 'WindowsMobile' else '其他' end `OS类型`," +
...
//      "from t_log)ret group by client   ")
//    ret.show()
//    ret.write.mode(SaveMode.Append)
//      .jdbc(getProperties()._2,"OSRpt",getProperties()
//        ._1)
    ...
    /**
      ...
      */
        ...
        val rdd: RDD[(String, List[Double])] = df.map(arr => {
          ...
          //key
          val client: Int = arr.getAs[Int]("client")
          val os:String =if(client==1) "Android" else if(client==2) "IOS" else if(client==3) "WindowsMobile"  else "其他"
          (os, RptUtils.request(reqm, proc) ++ RptUtils.click(reqm, iseff) ++ RptUtils.Ad(iseff, isbil, isbid, iswin, ado, winpri, adpay))
        }).rdd
